...
| # | Prerequisite | Detail |
|---|---|---|
| P1 | **Residency review first** (HG-0011) | A written record of what content classes may project to the SaaS floor. Default: references + status, never personal data; passport fields projected as summaries. Approved by data-protection + risk — as a signed record in git. This gates everything. **Drafted** as `notion-floor-residency-review.md`, with a **reviewer's brief** (`notion-floor-p1-reviewer-brief.md`) and a **sendable vendor questionnaire** (`notion-floor-p1-vendor-questionnaire.md`) to move it. Two of its residual risks are *unratable* until vendor Q7/Q8 and Q2/Q4 come back. |
