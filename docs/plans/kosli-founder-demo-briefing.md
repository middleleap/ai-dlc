...
| 3 | One traceable change with the explicit planning → mechanics transition | `harness/demo/run-demo.mjs --scenario meridian` + `harness/demo/meridian/` — one fictional Meridian obligation (`OB-AE-MTPOL-PSI-001`, payment-status integrity) → register rows → demo-scoped `PAYMENT-STATUS` control → executable check; first cut refused, repaired by the output of a recorded bounded agent run (`demo/meridian/agent-run/`: task, transcript, output, digests), rerun, record posted with the obligation id | Done against the fake; the repair is a *recorded* run replayed with its digests checked — not a live agent on stage, and the model is declared, not attested |
