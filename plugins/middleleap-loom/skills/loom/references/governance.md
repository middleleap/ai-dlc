...
| **HG-0004** | Broad agent credentials / secrets on disk | Least-privilege service identity for the agent + vaulted secrets. See `agent-credential-brokering.md` for the shape that holds the secret *outside* the agent rather than leasing it in, and `agent-egress-control.md` for the network identity half |
