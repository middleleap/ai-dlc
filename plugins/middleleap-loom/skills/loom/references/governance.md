...
| **HG-0004** | Broad agent credentials / secrets on disk | Least-privilege service identity for the agent + vaulted secrets. See `agent-runtime-security.md` for the credential-brokering shape, which holds the secret *outside* the agent rather than leasing it in |
...
| **HG-0011** | Agent LLM traffic and execution not residency-controlled | Onshore model gateway + pre-egress DLP + attested sandbox execution, where residency rules apply (`agent-runtime-security.md`) |
| **HG-0012** | The agent can reach "green" by retrieval, not derivation (mine `.git`, look up the fix) | Controlled build/eval runtime — sealed history + egress allow-list + an independent derivation-vs-retrieval audit (`agent-runtime-security.md`) |
