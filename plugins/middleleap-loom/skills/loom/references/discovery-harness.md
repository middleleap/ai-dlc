...
| 5d | **Define** | *product approval* | **New Product Approval pack** (regulated-bank profile — where the framed problem implies a customer-facing product or service change): the institution's Business Proposition Form, assembled beside the business case at problem fidelity, becomes the content of the `PA1` receipt at hand-off and, re-confirmed against what was built, the `PA2` receipt before launch; its conditions compile into the control plan and its post-implementation review is the lifecycle's 90-day PIR. UAE banks: the `npa-uae` skill in `middleleap-open-finance` | `npa-pack.md` + `npa-approved` record |
