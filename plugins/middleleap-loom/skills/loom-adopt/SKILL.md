...
| `governance/pilot-record.template.json` | `docs/governance/pilot-record.json` | core | The record of a declared supervised pilot — stage, caps, supervisor, rollback drill, and which adversarial rows from the playbook have been exercised. CORE, because the playbook it belongs to is core and now points at it; a core adopter who reads the playbook must find the file it names. Costs nothing until used: the gate arms on `status: active`, not on the file's presence, so an untouched copy is silent forever. THE HARNESS RUNS NO PILOT AND OBSERVES NO PRODUCTION — it reads the record, and SUPERVISED-PILOT stays Absent until a pilot has actually run |
...
| `governance/ai-governance.template.json` | `docs/governance/ai-governance.json` | governed | The consumer-impacting AI governance register: shipping model pin to accountable owner, meaningful human oversight, a non-AI route, bilingual disclosure, monitoring, incident response, privacy/security assessment and re-hashed stress-test evidence. Mandatory once a compiled plan requires ai_governance; records are mechanically validated and the runtime remains adopter-side |
| `governance/operating-model.template.json` | `docs/governance/operating-model.json` | governed | The institution-owned operating model: accountable executive and mandate, board oversight, change control, rollback and cease-use ownership, a complete RACI joined to identities, protected IAM bindings and independent re-performance through the selected external record. Mandatory for a high-tier regulated-bank plan. The gate validates the record and joins; appointments, IAM and organisational operation remain adopter-side |
| `governance/activation-plan.template.json` | `docs/governance/activation-plan.json` | full | The adopter activation campaign: four independent owners, one reference AI change, verified platform bypass observations, an active external-record provider (Kosli is the worked option), governed AI joins and a pilot state that cannot outrun its evidence. activating is a non-blocking work list; ready-for-pilot and later states fail closed. The gate coordinates records and observes no live system itself |
