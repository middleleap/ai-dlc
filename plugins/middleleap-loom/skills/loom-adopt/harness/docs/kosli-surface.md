...
| 3 | Can an attestation carry and verify the CI runner's OIDC token | **Partly** — `--user-data` carries anything up to 1MB, so the runner identity travels inside the envelope and PR6 verifies it in the gate. Whether Kosli verifies it natively is for the call |
