...
test('spec-tripwire: a shell command that rewrites the contract on a feature branch is denied', { skip: SKIP }, () => {
  ...
  try {
    for (const command of [
      "sed -i 's/v1/v2/' specs/openapi.yaml",
      'cat > specs/openapi.yaml <<EOF\nopenapi: 3.1.0\nEOF',
      'git mv specs/openapi.yaml specs/openapi-old.yaml',
      "node -e \"require('fs').writeFileSync('specs/openapi.yaml','')\"",
      'echo "" >> specs/openapi.yml',
    ]) {
...
test('spec-tripwire: claude/* branches are covered, and every known spelling of the contract path', { skip: SKIP }, () => {
  const repo = repoOn('claude/wizardly-brahmagupta');
  try {
    for (const file_path of ['specs/openapi.yaml', 'specs/openapi.yml', 'specs/openapi.json', '/abs/repo/specs/openapi.yaml']) {
...
test('spec-tripwire: a dedicated spec branch and main are the sanctioned lanes', { skip: SKIP }, () => {
  for (const branch of ['feature/STORY-7-spec-add-field', 'main']) {
    const repo = repoOn(branch);
    try {
      assert.ok(!denied(run('spec-tripwire.sh', { file_path: 'specs/openapi.yaml', content: 'x' }, repo)), `wrongly denied on ${branch}`);
      assert.ok(!denied(run('spec-tripwire.sh', { command: "sed -i 's/a/b/' specs/openapi.yaml" }, repo)), `wrongly denied on ${branch}`);
    } finally { rmSync(repo, { recursive: true, force: true }); }
  }
});
...
test('test-tripwire: the evasions of the plain it.skip regex are caught', { skip: SKIP }, () => {
