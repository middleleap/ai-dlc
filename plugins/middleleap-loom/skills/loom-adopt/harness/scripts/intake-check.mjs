...
import { validateRecord } from '../intake/record.mjs';
...
export function checkRecord(record, sources) {
  return validateRecord(record, sources.bank, sources.questions_digest);
}
...
export function run(cwd = process.cwd(), { recordPath = null, intakeDir = BUNDLE_INTAKE } = {}) {
  ...
  if (existsSync(rp)) {
    ...
    return { findings, notices, record: rp, summary: r.summary };
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (ri >= 0 && (!args[ri + 1] || args[ri + 1].startsWith('--'))) { process.stderr.write('--record requires a file path\n'); process.exit(2); }
  if (ri >= 0 && !existsSync(resolve(args[ri + 1]))) { process.stderr.write('Requested intake record does not exist\n'); process.exit(2); }
