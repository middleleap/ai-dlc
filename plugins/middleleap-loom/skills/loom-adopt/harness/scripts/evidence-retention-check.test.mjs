...
// conventional adopter never fails on this), and the SHIPPED TEMPLATE produces notices rather
// than findings until a compiled plan asks for the capability.
...
test('ER-R01 — a mounted file with no types object is a policy with no policy in it', () => {
  for (const doc of [{}, { types: [] }, { types: 'later' }, null]) {
    assert.ok(evaluate(doc).findings.some((f) => /ER-R01/.test(f)), JSON.stringify(doc));
  }
});
...
test('ER-R02 — the default is what unnamed types fall through to, so an unusable one is a finding', () => {
  for (const bad of [0, -1, 'seven', null]) {
    const { findings } = evaluate(policy({ tests: entry() }, { default_retention_years: bad }));
    assert.ok(findings.some((f) => /ER-R02/.test(f)), JSON.stringify(bad));
  }
  // Absent is allowed — every produced type must be named anyway.
  assert.deepEqual(evaluate({ types: { tests: entry() } }).findings, []);
});
...
test('ER-R04 — an entry that is not an object states nothing', () => {
  for (const bad of [7, 'seven', null, ['7y']]) {
    assert.ok(evaluate(policy({ tests: bad })).findings.some((f) => /ER-R04/.test(f)), JSON.stringify(bad));
  }
});
...
test('ER-R05 — neither a period nor a reason is the unretained state with extra JSON', () => {
  for (const years of [undefined, 0, -3, 'ten', null]) {
    const { findings } = evaluate(policy({ tests: entry({ retention_years: years, justification: undefined }) }));
    assert.ok(findings.some((f) => /ER-R05/.test(f)), JSON.stringify(years));
  }
  // An unreplaced ADOPT marker is not a justification.
  assert.ok(evaluate(policy({ tests: entry({ retention_years: undefined, justification: 'ADOPT: say why' }) }))
...
test('ER-R07 — an immutable_archive claim must say WHERE, per entry or once for the file', () => {
  ...
  // A non-boolean claim is always a finding — it is nobody's platform detail.
  assert.ok(evaluate(policy({ tests: entry({ immutable_archive: 'yes' }) })).findings.some((f) => /ER-R07.*true or false/.test(f)));
});
...
test('run() — the produced types come from the SEALED BUNDLE, and an uncovered one is named', () => {
  const dir = repo({ retention: policy({ tests: entry() }, { immutable_archive_ref: WORM }), bundleTypes: ['tests', 'sbom'] });
  try {
    const r = run(dir);
...
test('run() — a compiled plan\'s required_evidence counts as produced before any bundle exists', () => {
  const dir = repo({ evidence: ['tests', 'shariah-attestation'], retention: policy({ tests: entry() }, { immutable_archive_ref: WORM }) });
  try {
    const r = run(dir);
...
test('run() — a mounted policy that is not JSON fails rather than being skipped', () => {
  const dir = repo({ retention: '{ not json' });
  try {
    assert.ok(run(dir).findings.some((f) => /not valid JSON/.test(f)));
  } finally { clean(dir); }
});
