...
- **Agent runtime security** (`../loom/references/agent-runtime-security.md`): the credential-brokering egress proxy role — the shape that satisfies the "secrets on disk" half of this runbook by construction, the four peer instances, and what makes such a deployment inert.
