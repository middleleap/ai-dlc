...
export function renderDocument(spec, brand) {
  ...
  const body = `<div class="wrap">
    <header class="cover">
      <span class="wordmark">${esc(spec.wordmark || 'DEMO')}</span>
...
export function renderDeck(spec, brand) {
  ...
  const html = slides.map((s, i) => {
    const inner = s.cover
      ? `<span class="wordmark">${esc(spec.wordmark || 'DEMO')}</span><h1>${esc(s.title)}</h1>${s.note ? `<div class="sub">${esc(s.note)}</div>` : ''}`
...
export function renderPrototype(spec, brand) {
  ...
  const body = `<div class="wrap">
    <div class="topbar"><span class="wordmark">${esc(spec.wordmark || 'DEMO')}</span>
