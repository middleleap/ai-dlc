...
- **Register absent ⇒ `INSUFFICIENT_EVIDENCE`.** This agent judges against the Shari'ah structures register (`docs/governance/shariah-structures/`). If it is
...
- **`verdict`** is one of CONFORMANT or NON_CONFORMANT (the DRIFT line above maps to NON_CONFORMANT), or `INSUFFICIENT_EVIDENCE`. A pass-class verdict
