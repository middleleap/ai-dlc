...
- `research-log.md`: log every signal with a stable id (`S-001…`), its source, and type. File
  backing artifacts under `evidence/`. **Synthetic data only** in demo runs — tag illustrative
  signals `[synthetic]`. Zero real PII, ever.
...
> carries the marker. Never inline a raw hex/px/font.
