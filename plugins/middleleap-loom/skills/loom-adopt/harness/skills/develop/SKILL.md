...
write code, author tests, or edit the API contract — those are the delivery loop's, under its own
controls and four-eyes.
...
```
node discovery/gates/validate.mjs discovery/runs/<slug>    # must exit 0
```
...
- **Reuse-first / minimal-change** — compose existing platform primitives. Introduce nothing new.
- **Greenfield / ideal** — the best answer if cost were no object; names the ideal mechanism.
- **Buy-vs-integrate** — lean on an existing adapter or external capability over building.
...
conditions + **your project's hard-stops** (see `.claude/agents/hard-stop-reviewer.md` and the
binding rules in `CLAUDE.md`). Require each to return, as structured text: approach; how it meets
**each** success measure; how it honours **each** inherited data-governance condition and every
...
traceable record of *why this approach*, for the audit.
...
```yaml
- { id: <ID>, title: ..., status: pending, discovery: <slug>, sdr: docs/develop/<slug>.md, depends_on: [...] }
```

The `discovery: <slug>` field is what satisfies the **waist gate** — the item now traces to a
green hand-off, so the `next-story` Pick step accepts it. Keep stories small and independently
shippable; respect the hand-off's explicit out-of-scope.
