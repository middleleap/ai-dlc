...
- **Register absent ⇒ `INSUFFICIENT_EVIDENCE`.** This agent judges against the knowledge pins, the data-risk register, the obligations register and the lockfile. If it is
...
- **`verdict`** is one of HORIZON or CLEAR, or `INSUFFICIENT_EVIDENCE`. A pass-class verdict
