...
| `npa-uae` | New Product Approval for a CBUAE-regulated bank — the Business Proposition Form (33 fields with guidance, Word template in `assets/`), the regulatory anchor behind each field, sign-off routing, the BAU assessments, two worked Meridian Trust examples (the PFM view as a TPP, from the `cross-bank-money` discovery run; connected accounts for lending), and how the pack becomes the Loom's PA1/PA2 receipts with conditions compiled into the warp |
