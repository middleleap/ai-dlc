...
> **Last verified against sources: 31 August 2026 (errata3 register re-check found 3 corrections added since the 17 Aug full pass; prior passes 17 Aug / 13 Jul / 10 Jun 2026); wire-schema pass 3 September 2026 (Risk extension points, PII envelope, ControlParameters, errata3 folder contents, v2.2-rc1 detection — authenticated api-specs read; register not re-checked); ecosystem-watcher cadence re-check 14 September 2026 (register + repo-line checks via raw fetches — no drift found; register still lists 5 corrections under errata3, matching this file; doc-level Confluence register still at page version 5 / 8 Jul 2026; no new API Hub or Trust Framework release; `v2.2-rc1` still the highest pre-release line); re-check 21 September 2026 (still no drift — register + `erratas-registry.ts`/`api-hub-releases-registry.ts`/`trust-framework-releases-registry.ts` unchanged, `v2.2-rc1` still highest pre-release; `check_current.py` gained a raw.githubusercontent.com fallback probe so the tool's own repo-side check no longer goes blind when `api.github.com` is session-scope-blocked, which is the normal case for this skill's scheduled/automated runs).** Full audit trail, provenance, and resolved items: `references/verification-log.md`. Re-verify standards/errata level, pricing, and metrics before relying on time-sensitive figures — start with `python3 scripts/check_current.py` (it tracks the errata *number*, reports pre-release lines, cross-checks the register's "N corrections" count against this file's for the current errata number so a group that grows in place without a number bump is no longer a silent blind spot, **and now falls back to a narrow raw-file probe of a known pre-release frontier when the GitHub Tree API is unreachable**; still cross-check `references/standards-versions.md` for the full section-by-section detail).
