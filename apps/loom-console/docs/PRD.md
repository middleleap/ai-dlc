...
| | |
|---|---|
| **Status** | Draft v0.3 for implementation · 25 Sep 2026 |
...
|---|---|---|
| CON-01 | **Overview**: KPI strip (active runs, gate failures, approvals pending, obligations, controls validated); inbox preview; control maturity bar; integrity checks; pipeline across the double diamond; portfolio by strategic intent; activity | Every number traces to a field in `console.json`; each attention item names an owner resolved from the registry |
| CON-02 | **Inbox** (its own page, with a count in the navigation and a severity filter) is computed, never typed, from these rules: a gate failing on a reached stage; uncovered risks in live runs; approval rows from `approval-status` aggregated by change; runs awaiting reaction; NPA conditions with no record of being met; obligations pending owner verification; demo stand-in citations; radar reviews due within 30 days; unnamed seats for a profile a run uses; Factory Floor blocked | Adding or removing the triggering record adds or removes the item; there is no hard-coded item |
| CON-03 | **Context**, tabbed (BrainKit, risk register, obligations, strategy & radar, product profiles, coverage): BrainKit sections with digests, owners, approvals and the brainkit-check result; register categories, risks and controls; obligations table (owner role, holders, last verified, citation status, citing runs); product profiles with run counts; strategic intents; radar; runs × context matrix | brainkit-check result shown as an executed check; citation status is one of `cited`, `owner-verification-pending`, `demo-stand-in`, `missing` |
| CON-04 | **Discovery runs**: one row per run with stage track, gates (pass, failing, not reached), prototype, PRD or outcome, status and sponsor; status tabs with counts; text filter; product-owner Mine/All by sponsor | The stage algorithm is the one in §7.3; the filter never hides a run from a role that asks to "show all" |
| CON-05 | **Run detail**, laid out like an issue page (title, status, sponsor, stage; tabs Summary, Gates, Prototype, PRD, Data & risk): stage track; the full gate table with issues; the embedded committed wireframe with its prototype digest and reaction binding (bound, stale or none); PRD summary from `handoff.md` or the stop from `outcome.md`; NPA decision and conditions; hypotheses with per-stakeholder verdicts; data-governance verdict with risks, controls, obligations and uncovered risks; next step with the exact validator command | A stale reaction binding shows `STALE`; the wireframe shown is byte-identical to the committed `wireframe.html` |
...
| CON-07 | **View as**: a sidebar switch for the §3 roles. It sets the landing page and filters the inbox to the role's items; the product-owner view adds a sponsor picker. The per-role guide (registry holders, where to look, where the role decides with the HG citation and catalog state, what is not in the console) lives in the Help panel, not on the page. Deep link `#as-<role>` | Switching roles never changes the data, only landing page, inbox filter and the guide; every role can see everything |
...
| CON-09 | **Provenance UI**: every fact has a tag showing kind and file; clicking copies the path; the legend is in the Help panel | P2 test |
| CON-11 | **Product voice**: page titles are nouns (Overview, Inbox, Runs, Context, Installation) with a one-line factual meta line; no explanatory or persuasive copy on the page. Explanation (what a page shows, the provenance kinds, the maturity scale, snapshot details, the role guide) lives in a Help panel. Empty states are terse and factual ("No PRD · written when D1–D9 pass · 5/9 now"). Synthetic data is flagged by an environment bar, not prose | A copy review finds no sentence on a page that explains the method |
| CON-12 | **Light and dark themes** from the MiddleLeap tokens: follow the system setting until the viewer picks; the choice is remembered per browser; a host that sets `data-theme` on the root wins over the system | Both themes pass WCAG AA contrast for body text and badges |
